...
class stage5Scene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        lock = self.childNode(withName: "lock") as! SKSpriteNode
        lock.name = "lock"
...
//        特殊ブロックの設定
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if firstBody.node?.name == "ball" && secondBody.node?.name == "star" {
            ...
            //            ステージ6に移動する
            let scene = stage6Scene(fileNamed: "stage6")
            scene?.scaleMode = .aspectFill
...
//        鍵を取った時に南京錠が開く設定
        if firstBody.node?.name == "ball" && secondBody.node?.name == "key" {
            secondBody.node?.removeFromParent()
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        //        block1が左右に動く動き
        if block1.position.x >= 149{
            block1.run(SKAction.moveTo(x: 0, duration: 3))
        }else if block1.position.x <= 1{
            block1.run(SKAction.moveTo(x: 150, duration: 3))
        }
